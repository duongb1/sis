...
def preprocess_dataframe(df):
    ...
    text_cols = ['LYDO', 'HB_BENHLY', 'HB_BANTHAN', 'KB_TOANTHAN', 'KB_BOPHAN', 'MAICD', 'LABEL']
    ...
    for idx, row in df.iterrows():
        ...
        # 2. Create Label based on LABEL column first, falling back to MAICD
        if 'LABEL' in row and isinstance(row['LABEL'], str) and row['LABEL']:
            label = 1 if 'nhồi máu não cấp' in row['LABEL'].lower() else 0
        else:
            maicd = row.get('MAICD', '')
            label = 1 if 'i63' in str(maicd).lower() else 0
...
def process_pipeline():
    try:
        # Load Group 1: small.csv
        small_csv_path = os.path.join(base_dir, "data", "small.csv")
        print(f"Reading Group 1 ({small_csv_path})...")
        df_small = pd.read_csv(small_csv_path)
        
        print("Preprocessing Group 1...")
        proc_700 = preprocess_dataframe(df_small)
        
        # Export processed_700.csv
        proc_700.to_csv(output_700, index=False, encoding='utf-8-sig', quoting=csv.QUOTE_ALL)
        print(f"Exported: {output_700}")
        print(f"Total rows in processed_700.csv: {len(proc_700)}")
        
    except Exception as e:
        print(f"Error processing Group 1: {e}")

    try:
        # Load Group 2: large.csv
        large_csv_path = os.path.join(base_dir, "data", "large.csv")
        print(f"\nReading Group 2 ({large_csv_path})...")
        df_large = pd.read_csv(large_csv_path)
        
        print("Preprocessing Group 2...")
        proc_9937 = preprocess_dataframe(df_large)
        
        # Export processed_9937.csv
        proc_9937.to_csv(output_9937, index=False, encoding='utf-8-sig', quoting=csv.QUOTE_ALL)
        print(f"Exported: {output_9937}")
        print(f"Total rows in processed_9937.csv: {len(proc_9937)}")
